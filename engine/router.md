...
5. **State lives in files; judgment is single-threaded, execution may fan out.** discuss / plan / build / open / update / close are conducted by the main agent, which holds the whole picture. **Invariant:** judgment, gates, integration, and the living-spec fold stay single-threaded on the top model — they are never delegated. **Execution transport:** a *verified code-change task*'s execution may fan out to disposable per-task workers via the shared delegation transport when the runtime supports it, else it runs inline (`reference/risk.md ## Review transport`). Review and per-task build execution are the delegated procedures over one shared transport: review runs `agents/independent-reviewer.md` read-only, and build execution runs `agents/worker.md` (write + verify + commit). Both prefer delegated subagent dispatch when available and degrade to the inline role only when subagents are unavailable or dispatch fails for a runtime/tooling reason. A review trigger or user-approved build flow is an explicit request to use delegated transport when the runtime requires one. Pass just the task/review contract (the slug, command path, a summary of the latest artifact or the task row, and a pointer to the spec) — never the conversation history, and never a worker's compact report, as review evidence. Risk-cadence review (automatic, per `reference/risk.md ## Consequences`) and ad-hoc review (user-triggered via `レビューして` / `mochiflow-review`; see `reference/risk.md ## Ad-hoc review`) both use this transport.
