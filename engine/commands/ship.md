...
4. When the acceptance conditions in `reference/workflow.md ## AC Verification Matrix` all hold (matrix complete, every result is done-eligible, and the reviewer verdict recorded when `risk ≥ elevated`), the deterministic close-out is ready for `mochiflow ship {slug}`. The command re-runs final verification, updates eligible automated AC Matrix rows, sets `spec.yaml` `status: done`, `updated`, and `completed` (UTC ISO 8601, e.g. `2026-06-21T22:16:03Z`), regenerates `{index}`, moves the spec to `_done`, stages configured lifecycle paths, validates the staged diff, and creates the close-out commit. `completed` is the immutable completion time that orders the Done view in `INDEX.md`; the command sets it each time status becomes `done`. This is not a gate; `ship` is the only path that sets `done`.
...
5. Fold, archive, and close out — on the feature branch, before `mochiflow pr`.
   ...
   - Run `mochiflow ship {slug}`. Use `mochiflow ship --dry-run` first when you need to inspect blockers and planned lifecycle paths.
   - Manual fallback only when the CLI command is unavailable or this spec explicitly forbids dogfooding the new command: after setting `done`, moving `{specs_dir}/{slug}/` → `{specs_dir}/_done/{slug}/`, regenerating `{index}`, and folding ADR, stage with `git add -A {specs_dir} {index} {adr_paths...}`. Use the configured lifecycle parents/paths, never a pathspec that requires the moved-from `{specs_dir}/{slug}` to still exist. Validate with `git diff --cached --name-status -z` before committing.
   - The result is the **single close-out commit** per `reference/git.md ## Auto-commit and staging ### Ship close-out commit`, with an external-reviewer message (no spec slug, no AC IDs, no mochiflow vocabulary). Nothing is pushed to the base branch here.
