...
3. **Task loop**: repeat 3a–3e for each open task.
   ...
   - 3d. When the commit unit is reached, first mark every completed task in that commit unit as checked in `tasks.md` (`- [ ]` → `- [x]`) when `tasks.md` exists. Do not stage or commit while any completed task in the commit unit remains unchecked. Then commit per `reference/git.md ## Auto-commit`. Stage files explicitly.
