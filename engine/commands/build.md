...
3. **Task loop**: repeat 3a–3f for each open task.
   ...
   - 3e. When the commit unit is reached, first mark every completed task in that commit unit as checked in `tasks.md` (`- [ ]` → `- [x]`) when `tasks.md` exists. Do not stage or commit while any completed task in the commit unit remains unchecked. Then commit per `reference/git.md ## Auto-commit`. Stage files explicitly.
   - 3f. Follow the reviewer cadence in `reference/risk.md`; when required, run `agents/independent-reviewer.md` read-only via `reference/risk.md ## Review transport` (delegated subagent when available, otherwise inline reviewer role) and append the reviewer mode + verdict to `design.md ## Review Results`. For `critical`, this happens after each task.
