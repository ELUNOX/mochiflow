...
| --- | --- | --- |
| AC (pass/fail criteria) | `spec.md` | tasks header `対応 AC: AC-01` (ID only) |
...
- Optional `## 統合ログ` appended during build when the risk table calls for it (seam decisions, ownership, dead-code handling, handoff). Replaces the old separate journal file.

Do not write: AC tables (use tasks `対応 AC` + AC Matrix), concrete class/struct
...
- Dependency-ordered tasks; each: `対応 AC`, change area, task-specific stop conditions
...
contradicting the rationale in `spec.md`, tasks `対応 AC` covering `spec.md` AC,
