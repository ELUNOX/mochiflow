...
mod tests {
    ...
    #[test]
    fn kiro_worker_agent_is_full_file_managed_with_model_preserved() {
        // AC-12: the worker agent JSON is treated as a full-file managed target —
        // a user-set model is preserved and a model-only difference is not drift,
        // mirroring the reviewer agent's behavior.
        let out_rel = ".kiro/agents/spec-worker.json";
        let rendered = "{\n  \"name\": \"spec-worker\",\n  \"model\": \"claude-opus-4.8\",\n  \"_generated\": \"generated by mochiflow adapter=kiro version=1.0.0\"\n}\n";
        let current = "{\n  \"name\": \"spec-worker\",\n  \"model\": \"user-pinned-model\",\n  \"_generated\": \"generated by mochiflow adapter=kiro version=1.0.0\"\n}\n";

        // A model-only difference must not count as drift for the worker agent.
        assert!(kiro_agent_matches_rendered(out_rel, current, rendered));
        // The user-set model is preserved when writing the rendered output.
        let merged = preserve_kiro_agent_model(out_rel, Some(current), rendered);
        assert!(merged.contains("user-pinned-model"));
    }
}
