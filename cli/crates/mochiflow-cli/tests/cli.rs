...
fn init_blocked_json_exits_1_and_includes_candidate() {
    ...
    assert_eq!(
        ...
        Some(".mochiflow/state/adapters/.kiro/agents/spec-independent-reviewer.json"),
        "{stdout}"
    );
}
...
fn init_existing_structured_adapter_target_guidance_is_language_aware() {
    ...
    assert!(out.contains("Blocked"), "{out}");
    assert!(
        out.contains(".kiro/agents/spec-independent-reviewer.json は既に存在する構造化 adapter ファイルのため上書きしませんでした"),
        "{out}"
    );
    assert!(
        out.contains(".mochiflow/state/adapters/.kiro/agents/spec-independent-reviewer.json"),
        "{out}"
    );
}
...
fn init_existing_nested_adapter_target_writes_nested_candidate() {
    ...
    assert!(
        candidate_body.contains("spec-independent-reviewer"),
        "{candidate_body}"
    );
}
...
fn init_existing_adapter_targets_append_markdown_and_block_structured_files() {
    ...
    {
        let rel = ".kiro/agents/spec-independent-reviewer.json";
        let target = dir.path().join(rel);
        assert_eq!(
...
fn kiro_agent_non_model_override_is_adapter_drift() {
    ...
    let out = String::from_utf8_lossy(&result.get_output().stdout).into_owned();
    assert!(
        out.contains("DRIFT: .kiro/agents/spec-independent-reviewer.json"),
        "{out}"
    );
}
