...
| --- | --- | --- | --- | --- | --- | --- | --- |
| AC-01 | cli | automated | QA-01, QA-03, QA-05 | `config.rs`, mutation call sites, schema | PASS | path/schema/symlink tests + default profile<br>final verification: `cargo test --manifest-path cli/Cargo.toml && cargo fmt --manifest-path cli/Cargo.toml --all -- --check && cargo clippy --manifest-path cli/Cargo.toml --all-targets -- -D warnings && cargo run --manifest-path cli/Cargo.toml -- freeze --check` | caller paths remain separate |
| AC-02 | cli | automated | QA-01, QA-03, QA-05 | `adapter.rs`, `detach.rs` | PASS | malicious mapping/symlink tests + default profile<br>final verification: `cargo test --manifest-path cli/Cargo.toml && cargo fmt --manifest-path cli/Cargo.toml --all -- --check && cargo clippy --manifest-path cli/Cargo.toml --all-targets -- -D warnings && cargo run --manifest-path cli/Cargo.toml -- freeze --check` | shared checked resolver |
| AC-03 | cli | automated | QA-01 | `spec_meta.rs` | PASS | malformed quote/catch-unwind tests<br>final verification: `cargo test --manifest-path cli/Cargo.toml && cargo fmt --manifest-path cli/Cargo.toml --all -- --check && cargo clippy --manifest-path cli/Cargo.toml --all-targets -- -D warnings && cargo run --manifest-path cli/Cargo.toml -- freeze --check` | empty quotes accepted |
| AC-04 | cli | automated + AI-observed | QA-02, QA-07 | initialization docs | PASS | conformance + direct inspection | removed flag absent |
| AC-05 | cli | automated | QA-01, QA-06, QA-07 | `index.rs` | PASS | golden Markdown/JSON tests<br>final verification: `cargo test --manifest-path cli/Cargo.toml && cargo fmt --manifest-path cli/Cargo.toml --all -- --check && cargo clippy --manifest-path cli/Cargo.toml --all-targets -- -D warnings && cargo run --manifest-path cli/Cargo.toml -- freeze --check` | one immutable snapshot |
...
| AC-07 | cli | automated | QA-01, QA-04, QA-07 | `accept.rs` | PASS | staging/dirty-state integration + default profile<br>final verification: `cargo test --manifest-path cli/Cargo.toml && cargo fmt --manifest-path cli/Cargo.toml --all -- --check && cargo clippy --manifest-path cli/Cargo.toml --all-targets -- -D warnings && cargo run --manifest-path cli/Cargo.toml -- freeze --check` | exact retry only |
...
| AC-09 | cli | automated | QA-01, QA-06 | `freeze.rs` | PASS | traversal tests + freeze check<br>final verification: `cargo test --manifest-path cli/Cargo.toml && cargo fmt --manifest-path cli/Cargo.toml --all -- --check && cargo clippy --manifest-path cli/Cargo.toml --all-targets -- -D warnings && cargo run --manifest-path cli/Cargo.toml -- freeze --check` | compute before write |
| AC-10 | cli | automated | QA-01, QA-04, QA-06 | `upgrade.rs`, `doctor.rs` | PASS | drift/force integration + default profile<br>final verification: `cargo test --manifest-path cli/Cargo.toml && cargo fmt --manifest-path cli/Cargo.toml --all -- --check && cargo clippy --manifest-path cli/Cargo.toml --all-targets -- -D warnings && cargo run --manifest-path cli/Cargo.toml -- freeze --check` | backup paths reported |
...
| AC-12 | cli | automated | QA-01, QA-05, QA-07 | semantic conformance parser | PASS | 198 conformance tests + cargo-deny<br>final verification: `cargo test --manifest-path cli/Cargo.toml && cargo fmt --manifest-path cli/Cargo.toml --all -- --check && cargo clippy --manifest-path cli/Cargo.toml --all-targets -- -D warnings && cargo run --manifest-path cli/Cargo.toml -- freeze --check` | prose guards retained |
