...
| --- | --- | --- | --- | --- | --- | --- | --- |
| AC-01 | cli | AI-observed | QA-05 | `engine/reference/risk.md` | UNVERIFIED | | shared judgment stated once |
| AC-02 | cli | AI-observed | QA-01, QA-06 | `engine/commands/update.md`, `engine/commands/build.md` | UNVERIFIED | | apply-commit-hold, no auto push/accept/review |
| AC-03 | cli | automated | QA-04, QA-05 | `engine/reference/risk.md`, `engine/templates/spec/design.md` | UNVERIFIED | | `Reviewed through: <sha>` on its own line below `Verdict:` |
| AC-04 | cli | automated | QA-01, QA-05 | `engine/commands/update.md`, `cli/crates/mochiflow-cli/tests/conformance.rs` | UNVERIFIED | | hold vs. explicit finalize split |
| AC-05 | cli | AI-observed | QA-01, QA-04 | `engine/commands/update.md` | UNVERIFIED | | review-if-stale once (code commits only), then push |
| AC-06 | cli | AI-observed | QA-08 | `engine/commands/open.md` | UNVERIFIED | | accept-gate (step 6) trigger, fires even with no QA round-trip; code commits only |
| AC-07 | cli | automated | QA-05 | `engine/commands/build.md`, `cli/crates/mochiflow-cli/tests/conformance.rs` | UNVERIFIED | | post-completion window + unchanged plan-routing |
| AC-08 | cli | automated | QA-05 | `engine/reference/workflow.md`, `engine/agents/change-reviewer.md` | UNVERIFIED | | guard row: these two files are checked for absence of change; `build.md`'s task-structure rule and the two delivery gates are checked for preserved wording (not file-level non-change — T-004 edits `build.md` for an unrelated addition) |
| AC-09 | cli | automated | QA-05 | `cli/crates/mochiflow-cli/tests/conformance.rs` | UNVERIFIED | | full regression + new pinned tests |
| AC-10 | cli | automated | QA-07 | `engine/MANIFEST.json`, `cli/` | UNVERIFIED | | dogfood sync + default verify + lint + doctor |
| AC-11 | cli | automated | QA-05 | `engine/router.md`, `cli/crates/mochiflow-cli/tests/conformance.rs` | UNVERIFIED | | one-line PR-feedback-routing wording fix only |
