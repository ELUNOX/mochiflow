...
| AC-04 | cli | automated + human QA | QA-01, QA-05 | `engine/commands/update.md`, `cli/crates/mochiflow-cli/tests/conformance.rs` | PENDING_HUMAN | Automated evidence passed via `update_holds_fixes_until_explicit_finalize_signal`, `router_update_summary_matches_hold_by_default`, and `pr_feedback_routes_to_update_without_restore` in `cargo test --manifest-path cli/Cargo.toml`; human QA-01 remains for `open` | hold vs. explicit finalize split |
| AC-05 | cli | AI-observed + human QA | QA-01, QA-04 | `engine/commands/update.md` | PENDING_HUMAN | Automated/doc evidence passed via `update_holds_fixes_until_explicit_finalize_signal` in `cargo test --manifest-path cli/Cargo.toml`; human QA-01 remains for `open` | review-if-stale once (code commits only), then push |
| AC-06 | cli | AI-observed + human QA | QA-08 | `engine/commands/open.md` | PENDING_HUMAN | Automated/doc evidence passed via `open_generalizes_freshness_trigger_to_reviewed_through` and `open_orders_acceptance_fold_accept_pr_gate` in `cargo test --manifest-path cli/Cargo.toml`; human QA-08 remains for `open` | accept-gate (step 6) trigger, fires even with no QA round-trip; code commits only |
