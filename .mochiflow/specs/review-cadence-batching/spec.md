...
| AC-02 | cli | AI-observed + human QA | QA-01, QA-06 | `engine/commands/update.md`, `engine/commands/build.md` | CONFIRMED | Automated/doc evidence passed via `update_holds_fixes_until_explicit_finalize_signal` and `build_documents_post_completion_bounded_fix_window` in `cargo test --manifest-path cli/Cargo.toml`; human QA-01/QA-06 confirmed in `open` QA round-trip on 2026-07-03 | apply-commit-hold, no auto push/accept/review |
| AC-03 | cli | automated | QA-04, QA-05 | `engine/reference/risk.md`, `engine/templates/spec/design.md`, `engine/commands/build.md` | PASS | `risk_defines_shared_bounded_fix_judgment_and_reviewed_through` and `build_commit_cadence_is_task_based_not_risk_based` in `cargo test --manifest-path cli/Cargo.toml`; AI-observed design template and build command wording<br>final verification: `cargo test --manifest-path cli/Cargo.toml && cargo fmt --manifest-path cli/Cargo.toml --all -- --check && cargo clippy --manifest-path cli/Cargo.toml --all-targets -- -D warnings && cargo run --manifest-path cli/Cargo.toml -- freeze --check` | `Reviewed through: <sha>` on its own line below `Verdict:` |
| AC-04 | cli | automated + human QA | QA-01, QA-05 | `engine/commands/update.md`, `cli/crates/mochiflow-cli/tests/conformance.rs` | CONFIRMED | Automated evidence passed via `update_holds_fixes_until_explicit_finalize_signal`, `router_update_summary_matches_hold_by_default`, and `pr_feedback_routes_to_update_without_restore` in `cargo test --manifest-path cli/Cargo.toml`; human QA-01 confirmed in `open` QA round-trip on 2026-07-03 | hold vs. explicit finalize split |
| AC-05 | cli | AI-observed + human QA | QA-01, QA-04 | `engine/commands/update.md` | CONFIRMED | Automated/doc evidence passed via `update_holds_fixes_until_explicit_finalize_signal` in `cargo test --manifest-path cli/Cargo.toml`; human QA-01 confirmed in `open` QA round-trip on 2026-07-03 | review-if-stale once (code commits only), then push |
| AC-06 | cli | AI-observed + human QA | QA-08 | `engine/commands/open.md` | CONFIRMED | Automated/doc evidence passed via `open_generalizes_freshness_trigger_to_reviewed_through` and `open_orders_acceptance_fold_accept_pr_gate` in `cargo test --manifest-path cli/Cargo.toml`; human QA-08 confirmed in `open` QA round-trip on 2026-07-03 | accept-gate (step 6) trigger, fires even with no QA round-trip; code commits only |
| AC-07 | cli | automated | QA-05 | `engine/commands/build.md`, `cli/crates/mochiflow-cli/tests/conformance.rs` | PASS | `build_documents_post_completion_bounded_fix_window` and `build_ends_at_approved_without_pr_or_move` in `cargo test --manifest-path cli/Cargo.toml`<br>final verification: `cargo test --manifest-path cli/Cargo.toml && cargo fmt --manifest-path cli/Cargo.toml --all -- --check && cargo clippy --manifest-path cli/Cargo.toml --all-targets -- -D warnings && cargo run --manifest-path cli/Cargo.toml -- freeze --check` | post-completion window + unchanged plan-routing |
| AC-08 | cli | automated | QA-05 | `engine/reference/workflow.md`, `engine/agents/change-reviewer.md`, `engine/commands/build.md` | PASS | `open_orders_acceptance_fold_accept_pr_gate`, `build_documents_post_completion_bounded_fix_window`, and `canonical_reviewers_grounded_adversary_contract_is_pinned` in `cargo test --manifest-path cli/Cargo.toml`; AI-observed no edits to `engine/agents/change-reviewer.md`<br>final verification: `cargo test --manifest-path cli/Cargo.toml && cargo fmt --manifest-path cli/Cargo.toml --all -- --check && cargo clippy --manifest-path cli/Cargo.toml --all-targets -- -D warnings && cargo run --manifest-path cli/Cargo.toml -- freeze --check` | task-structure rule, two delivery gates, and reviewer contract preserved |
| AC-09 | cli | automated | QA-05 | `cli/crates/mochiflow-cli/tests/conformance.rs` | PASS | `cargo test --manifest-path cli/Cargo.toml` PASS, including pre-existing routing/order guards and new conformance tests<br>final verification: `cargo test --manifest-path cli/Cargo.toml && cargo fmt --manifest-path cli/Cargo.toml --all -- --check && cargo clippy --manifest-path cli/Cargo.toml --all-targets -- -D warnings && cargo run --manifest-path cli/Cargo.toml -- freeze --check` | full regression + new pinned tests |
| AC-10 | cli | automated | QA-07 | `engine/MANIFEST.json`, `cli/` | PASS | `mochiflow freeze && mochiflow upgrade --source engine && mochiflow adapter generate --check` PASS; `cargo test --manifest-path cli/Cargo.toml && cargo fmt --manifest-path cli/Cargo.toml --all -- --check && cargo clippy --manifest-path cli/Cargo.toml --all-targets -- -D warnings && cargo run --manifest-path cli/Cargo.toml -- freeze --check` PASS; `mochiflow lint --spec review-cadence-batching` PASS; `mochiflow doctor` 0 fail<br>final verification: `cargo test --manifest-path cli/Cargo.toml && cargo fmt --manifest-path cli/Cargo.toml --all -- --check && cargo clippy --manifest-path cli/Cargo.toml --all-targets -- -D warnings && cargo run --manifest-path cli/Cargo.toml -- freeze --check` | dogfood sync + default verify + lint + doctor |
| AC-11 | cli | automated | QA-05 | `engine/router.md`, `cli/crates/mochiflow-cli/tests/conformance.rs` | PASS | `router_update_summary_matches_hold_by_default` and `pr_feedback_routes_to_update_without_restore` in `cargo test --manifest-path cli/Cargo.toml`; AI-observed two router summary edits only<br>final verification: `cargo test --manifest-path cli/Cargo.toml && cargo fmt --manifest-path cli/Cargo.toml --all -- --check && cargo clippy --manifest-path cli/Cargo.toml --all-targets -- -D warnings && cargo run --manifest-path cli/Cargo.toml -- freeze --check` | PR-feedback-routing wording aligned with hold/finalize |
