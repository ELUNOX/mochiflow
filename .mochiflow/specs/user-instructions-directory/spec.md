...
| --- | --- | --- | --- | --- | --- | --- | --- |
| AC-01 | cli | automated | Init fixture assertions; QA-01, QA-04 | `cli/crates/mochiflow-cli/src/main.rs`; `cli/crates/mochiflow-core/src/init.rs`; `cli/crates/mochiflow-cli/tests/cli.rs`; `cli/crates/mochiflow-cli/tests/first_run.rs` | PASS | `cargo test --manifest-path cli/Cargo.toml`; `init_scaffolds_deterministically`; `init_existing_config_preserves_config_but_still_scaffolds_init_paths`; `init_fails_when_instruction_paths_are_files`; `init_fails_when_instructions_readme_path_is_directory`<br>final verification: `cargo test --manifest-path cli/Cargo.toml && cargo fmt --manifest-path cli/Cargo.toml --all -- --check && cargo clippy --manifest-path cli/Cargo.toml --all-targets -- -D warnings && cargo run --manifest-path cli/Cargo.toml -- freeze --check` | Includes fresh/existing human-readable and JSON/dry-run output expectations plus path-collision safety. |
| AC-02 | cli | automated | Repeated and forced init preservation test; QA-02 | `cli/crates/mochiflow-cli/src/main.rs`; `cli/crates/mochiflow-core/src/init.rs`; `cli/crates/mochiflow-cli/tests/cli.rs` | PASS | `cargo test --manifest-path cli/Cargo.toml`; `init_preserves_user_instruction_files_even_with_force`; `init_is_idempotent_nondestructive`<br>final verification: `cargo test --manifest-path cli/Cargo.toml && cargo fmt --manifest-path cli/Cargo.toml --all -- --check && cargo clippy --manifest-path cli/Cargo.toml --all-targets -- -D warnings && cargo run --manifest-path cli/Cargo.toml -- freeze --check` | Locks the README as user-owned after creation. |
| AC-03 | cli | automated | Custom ignore and Git status fixture; QA-03 | `cli/crates/mochiflow-core/src/init.rs`; `.mochiflow/.gitignore`; `cli/crates/mochiflow-cli/tests/cli.rs` | PASS | `cargo test --manifest-path cli/Cargo.toml`; `init_appends_local_instructions_ignore_rule_without_hiding_shared_files`; `init_appends_missing_local_instructions_ignore_rule_to_custom_file`; `init_gitignore_preserve_then_force`; repo `.mochiflow/.gitignore` contains `instructions.local/` and not `instructions/`<br>final verification: `cargo test --manifest-path cli/Cargo.toml && cargo fmt --manifest-path cli/Cargo.toml --all -- --check && cargo clippy --manifest-path cli/Cargo.toml --all-targets -- -D warnings && cargo run --manifest-path cli/Cargo.toml -- freeze --check` | Existing custom content is preserved without force. |
| AC-04 | cli | automated | Init dispatch plus join/upgrade/diagnostic regression and standing-contract content checks; QA-06, QA-07, QA-08 | `cli/crates/mochiflow-cli/src/main.rs`; `cli/crates/mochiflow-cli/tests/cli.rs`; `cli/crates/mochiflow-cli/tests/conformance.rs` | PASS | `cargo test --manifest-path cli/Cargo.toml`; `join_and_upgrade_do_not_backfill_user_instruction_dirs`; `user_instruction_dirs_are_not_standing_or_adapter_inputs`; `cargo run --manifest-path cli/Cargo.toml -- adapter generate --check`; `cargo run --manifest-path cli/Cargo.toml -- doctor`<br>final verification: `cargo test --manifest-path cli/Cargo.toml && cargo fmt --manifest-path cli/Cargo.toml --all -- --check && cargo clippy --manifest-path cli/Cargo.toml --all-targets -- -D warnings && cargo run --manifest-path cli/Cargo.toml -- freeze --check` | Explicit join remains separate and does not backfill; no config, engine, or adapter implementation file is added. |
| AC-05 | cli | automated | Detach/purge fixture assertions; QA-05 | `cli/crates/mochiflow-core/src/detach.rs`; `cli/crates/mochiflow-cli/tests/cli.rs` | PASS | `cargo test --manifest-path cli/Cargo.toml`; `detach_removes_managed_block_and_runtime_but_preserves_project_data`; `detach_purge_requires_exact_confirmation_and_deletes_all_project_data`; `detach_json_purge_warning_stays_out_of_stdout_document`<br>final verification: `cargo test --manifest-path cli/Cargo.toml && cargo fmt --manifest-path cli/Cargo.toml --all -- --check && cargo clippy --manifest-path cli/Cargo.toml --all-targets -- -D warnings && cargo run --manifest-path cli/Cargo.toml -- freeze --check` | Covers text and structured reporting without changing the confirmation phrase. |
| AC-06 | cli | automated / AI-observed | Output/golden assertions and documentation read-through; QA-01, QA-07 | `cli/crates/mochiflow-cli/src/main.rs`; `cli/crates/mochiflow-core/src/init.rs`; `cli/crates/mochiflow-cli/tests/golden/init_npm.json`; `README.md`; `README.ja.md`; `docs/getting-started.md`; `docs/configuration.md` | PASS | `cargo test --manifest-path cli/Cargo.toml`; `qa06_json_matches_golden_and_stdout_is_pure_json`; `init_scaffolds_deterministically`; docs inspected in `README.md`, `README.ja.md`, `docs/getting-started.md`, and `docs/configuration.md` | Seeded README content and every explicit init presentation share the same contract. |
| AC-07 | cli | AI-observed | Inspect ignored local files after migration; QA-07 | `.mochiflow/constitution.local.md`; `.mochiflow/instructions.local/release.md` | PASS | `git status --short --ignored .mochiflow/constitution.local.md .mochiflow/instructions.local/release.md`; `git check-ignore -v .mochiflow/instructions.local/release.md .mochiflow/constitution.local.md`; inspected `.mochiflow/instructions.local/release.md` and constitution local stub | Local working-copy evidence only; neither path is staged. |
