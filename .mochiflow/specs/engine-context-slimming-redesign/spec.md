...
| AC-05 | cli | automated + independent review | QA-06, QA-07; reviewer profile/resource assertions | `engine/agents/**`; Kiro reviewer templates | UNVERIFIED | The 2026-07-09 inline change review correctly failed the pre-fix lifecycle-owner reachability gap. The bounded fix is verified, but the requested one-round budget does not include a clean post-fix review. | A fresh full-diff change review is still required before acceptance. |
...
| AC-07 | cli | automated | QA-04, QA-05; bundled/source upgrade and adapter collision fixtures | upgrade/adapter behavior; engine/adapters layout | PASS | Bundled/source upgrade, adapter collision/cleanup, deterministic generation, `mochiflow adapter generate --check`, and source/vendored engine comparison passed on 2026-07-09. | Deprecated target cleanup remains code-owned. |
| AC-08 | cli | automated | QA-04, QA-06, QA-07; configured verification, frozen-surface no-change check, and dogfood sync | generated/vendored artifacts and spec evidence | UNVERIFIED | Full CLI tests, fmt, clippy, freeze check, adapter drift check, spec lint, and dogfood synchronization pass after the bounded fix. The fresh post-fix change review remains outstanding with AC-05. | Frozen schema/version files, budget tests, and measurement commands remain out of scope. |
