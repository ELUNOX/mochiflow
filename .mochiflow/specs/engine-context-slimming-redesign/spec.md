...
| --- | --- | --- | --- | --- | --- | --- | --- |
| AC-01 | cli | automated + AI-observed | QA-02, QA-04; adapter outputs and non-frozen terminology checks | `engine/router.md`; `engine/adapters/**`; config/init sources; T-008 live-claim cleanup | PASS | `live_engine_foundational_context_claims_are_conditional`, `adapters_separate_standing_inputs_from_load_on_demand`, and `frozen_schema_and_version_are_unchanged_by_this_feature` passed in the 2026-07-09 full CLI run; direct Kiro/AGENTS observation is recorded in `design.md`. | The frozen schema stays intentionally unchanged. |
| AC-02 | cli | automated + AI-observed | QA-01, QA-03; routing conformance plus hostile-content behavioral observations | `engine/router.md`; `engine/commands/*.md`; adapters; conformance tests | PASS | Router branch, route-ownership, and command-frontmatter conformance passed in the 2026-07-09 full CLI run; QA-01/QA-03 observations are recorded in `design.md ## Behavioral Observation Matrix`. | Command procedure names and body references remain allowed; the activation catalog does not. |
| AC-03 | cli | automated + inspection | QA-07; declared-reference existence and stale-path search | `engine/reference/**`; engine documentation | PASS | `removed_monolith_and_wrapper_paths_are_absent`, `engine_frontmatter_declared_paths_exist`, and `migrated_invariants_have_a_single_owner` passed in the 2026-07-09 full CLI run. | Historical specs, ADRs, fixtures, and explicit deprecated generated-target cleanup remain allowed exceptions. |
| AC-04 | cli | automated + AI-observed | QA-01, QA-02, QA-06; load-structure checks plus conditional-load observation matrix | `engine/commands/*.md`; selected templates | PASS | Staged-load, declared-path, bounded-fix reachability, and lifecycle-owner reachability conformance passed after the review fix; conditional-load observations are recorded in `design.md`. | Required and selected conditional loads remain file-level. |
| AC-05 | cli | automated + independent review | QA-06, QA-07; reviewer profile/resource assertions | `engine/agents/**`; Kiro reviewer templates | PASS | Fresh delegated `change-reviewer` review through `fe9a752` found no Critical or High finding; the full CLI verification, adapter drift check, and source-to-vendored comparison passed on 2026-07-10. | One Medium documentation-scope comment remains: T-006's historical `Files:` list omits its required backlog seed. |
| AC-06 | cli | automated + inspection | QA-07; owner-map and duplicate-contract review; single-ownership structural check | `engine/reference/**`; commands; templates | PASS | Single-owner, standing-router, adapter-policy, and removed-owner conformance passed in the 2026-07-09 full CLI run; direct owner-map inspection found no remaining duplicate owner. | One owner plus local mutation guards; no size/token budget. |
| AC-07 | cli | automated | QA-04, QA-05; bundled/source upgrade and adapter collision fixtures | upgrade/adapter behavior; engine/adapters layout | PASS | Bundled/source upgrade, adapter collision/cleanup, deterministic generation, `mochiflow adapter generate --check`, and source/vendored engine comparison passed on 2026-07-09.<br>final verification: `cargo test --manifest-path cli/Cargo.toml && cargo fmt --manifest-path cli/Cargo.toml --all -- --check && cargo clippy --manifest-path cli/Cargo.toml --all-targets -- -D warnings && cargo run --manifest-path cli/Cargo.toml -- freeze --check` | Deprecated target cleanup remains code-owned. |
| AC-08 | cli | automated | QA-04, QA-06, QA-07; configured verification, frozen-surface no-change check, and dogfood sync | generated/vendored artifacts and spec evidence | PASS | Full configured verification (1 unit, 79 CLI, 193 conformance, 7 first-run, 24 PR, and 138 core tests), fmt, clippy, freeze check, adapter drift check, spec lint, ADR lint, and diff check passed on 2026-07-10.<br>final verification: `cargo test --manifest-path cli/Cargo.toml && cargo fmt --manifest-path cli/Cargo.toml --all -- --check && cargo clippy --manifest-path cli/Cargo.toml --all-targets -- -D warnings && cargo run --manifest-path cli/Cargo.toml -- freeze --check` | Frozen schema/version files, budget tests, and measurement commands remain out of scope. |
