...
- [ ] T-001 [AC-06] Extract pure builders into mochiflow-core (hash, version reader, manifest builder, repo-root)
  - Depends on: none
...
- [ ] T-002 [AC-02] [AC-03] [AC-04] [AC-05] Implement freeze core (write + check, idempotent)
  - Depends on: T-001
...
- [ ] T-003 [AC-01] [AC-07] Wire `mochiflow freeze` into the CLI and remove `engine manifest`
  - Depends on: T-002
...
- [ ] T-004 [AC-08] Collapse the version gate to a single rule
  - Depends on: T-001
...
- [ ] T-005 [P] [AC-09] Replace the literal version test with a dynamic one
  - Depends on: none
...
- [ ] T-006 [AC-10] Add `freeze --check` to CI
  - Depends on: T-003
...
- [ ] T-007 [AC-11] Update contributor docs and remove the drift warning
  - Depends on: T-003
...
- [ ] T-008 [chore: full gate] Run the complete verification gate and self-freeze
