...
| --- | --- | --- | --- | --- | --- |
| QA-01 | P1, P7 | cli | Human-operated | Read the updated templates and guidance as a first-time author and a spec skeptic; check that dimensions are understandable, not role-play personas, and still trace through `QA-XX` to the AC Matrix. | The authoring path is understandable and internally consistent. |
| QA-02 | P3 | cli | Human-operated | Review the updated reviewer contracts for privilege creep, conversation-history dependence, and reviewer-as-author wording. | Reviewers remain read-only auditors and do not author QA, fix findings, stage, commit, or create PR metadata. |
| QA-03 | P4 | cli | Automated | N/A check: confirm the change touches no persisted product data or runtime state. | N/A: engine docs/templates/tests only; no data persistence path exists. |
| QA-04 | P5 | cli | Automated | N/A check: confirm archived specs are not migrated and existing legacy persona wording remains acceptable where already archived. | N/A: no archival migration is required; compatibility is preserved. |
| QA-05 | P6 | cli | Automated | Run conformance and surface verification after engine/template/adapter updates. | Existing workflow contracts still pass, including reviewer transport and adapter generation checks. |
| QA-06 | P2 | cli | Human-operated | Check the post-implementation review contract against a large-refactor scenario and a mechanical-rename scenario. | The contract distinguishes pure/mechanical refactor from semantic refactor and requires behavior-preservation evidence where needed. |
...
| --- | --- | --- | --- | --- | --- | --- | --- |
| AC-01 | cli | automated | `cargo test --manifest-path cli/Cargo.toml --test conformance`; QA-01 | build fills changed files | UNVERIFIED | build records command output |  |
| AC-02 | cli | automated | `cargo test --manifest-path cli/Cargo.toml --test conformance`; QA-01 | build fills changed files | UNVERIFIED | build records command output |  |
| AC-03 | cli | automated | `cargo test --manifest-path cli/Cargo.toml --test conformance`; QA-01 | build fills changed files | UNVERIFIED | build records command output |  |
| AC-04 | cli | automated | `cargo test --manifest-path cli/Cargo.toml --test conformance`; QA-02 | build fills changed files | UNVERIFIED | build records command output |  |
| AC-05 | cli | automated | `cargo test --manifest-path cli/Cargo.toml --test conformance`; QA-02, QA-06 | build fills changed files | UNVERIFIED | build records command output |  |
| AC-06 | cli | automated | `cargo test --manifest-path cli/Cargo.toml --test conformance`; QA-02 | build fills changed files | UNVERIFIED | build records command output |  |
| AC-07 | cli | automated | `cargo test --manifest-path cli/Cargo.toml --test conformance`; QA-02 | build fills changed files | UNVERIFIED | build records command output |  |
| AC-08 | cli | automated | `mochiflow adapter generate --check`; conformance adapter tests; QA-02 | build fills changed files | UNVERIFIED | build records command output |  |
| AC-09 | cli | automated | `mochiflow freeze --check`; `mochiflow adapter generate --check`; `mochiflow lint --spec qa-attack-dimensions-redesign`; surface `default` verification | build fills changed files | UNVERIFIED | build records command output |  |
