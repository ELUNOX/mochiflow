...
| --- | --- | --- | --- | --- | --- | --- | --- |
| AC-01 | cli | automated | QA-01, QA-05 | `contracts/agent-context.schema.json`, `inspect.rs`, CLI conformance tests | PASS | `schema_agent_context_accepts_all_result_variants`; `inspect_json_is_one_schema_valid_document`<br>final verification: `cargo test --manifest-path cli/Cargo.toml && cargo fmt --manifest-path cli/Cargo.toml --all -- --check && cargo clippy --manifest-path cli/Cargo.toml --all-targets -- -D warnings && cargo run --manifest-path cli/Cargo.toml -- freeze --check` | repository and spec variants |
| AC-02 | cli | automated | QA-01, QA-03 | `inspect.rs`, structured lint and persistence APIs | PASS | live schema-valid detail test asserts structured payload; schema fixtures pin observation qualities<br>final verification: `cargo test --manifest-path cli/Cargo.toml && cargo fmt --manifest-path cli/Cargo.toml --all -- --check && cargo clippy --manifest-path cli/Cargo.toml --all-targets -- -D warnings && cargo run --manifest-path cli/Cargo.toml -- freeze --check` | no content embedding |
...
| AC-04 | cli | automated | QA-06 | delivery observation snapshot and warning model | PASS | `failed_git_batches_remain_unavailable`; `provider_truncation_never_becomes_known_false`<br>final verification: `cargo test --manifest-path cli/Cargo.toml && cargo fmt --manifest-path cli/Cargo.toml --all -- --check && cargo clippy --manifest-path cli/Cargo.toml --all-targets -- -D warnings && cargo run --manifest-path cli/Cargo.toml -- freeze --check` | tri-state facts |
| AC-05 | cli | automated | QA-01, QA-02 | inspect presenters and config/error dispatch | PASS | `inspect_json_is_one_schema_valid_document`; `inspect_missing_slug_returns_structured_error`; Japanese human-output test<br>final verification: `cargo test --manifest-path cli/Cargo.toml && cargo fmt --manifest-path cli/Cargo.toml --all -- --check && cargo clippy --manifest-path cli/Cargo.toml --all-targets -- -D warnings && cargo run --manifest-path cli/Cargo.toml -- freeze --check` | partial exits 1 |
| AC-06 | cli | automated | QA-04 | inspect command and fetch boundary | PASS | read-only snapshot construction; full status/index mutation regression suite<br>final verification: `cargo test --manifest-path cli/Cargo.toml && cargo fmt --manifest-path cli/Cargo.toml --all -- --check && cargo clippy --manifest-path cli/Cargo.toml --all-targets -- -D warnings && cargo run --manifest-path cli/Cargo.toml -- freeze --check` | no persistent cache |
| AC-07 | cli | automated | QA-03 | serialization and diagnostic sanitization | PASS | negative absolute-path fixture; path-like slug structured-error behavior<br>final verification: `cargo test --manifest-path cli/Cargo.toml && cargo fmt --manifest-path cli/Cargo.toml --all -- --check && cargo clippy --manifest-path cli/Cargo.toml --all-targets -- -D warnings && cargo run --manifest-path cli/Cargo.toml -- freeze --check` | repository-relative paths |
| AC-08 | cli | automated | QA-05, QA-07 | `status.rs`, `index.rs`, `ready`, shared snapshot | PASS | unchanged index golden, status read-only tests, ready conformance, full default suite<br>final verification: `cargo test --manifest-path cli/Cargo.toml && cargo fmt --manifest-path cli/Cargo.toml --all -- --check && cargo clippy --manifest-path cli/Cargo.toml --all-targets -- -D warnings && cargo run --manifest-path cli/Cargo.toml -- freeze --check` | existing JSON unchanged |
| AC-09 | cli | automated | QA-06 | batched Git/provider collector | PASS | `repository_probe_count_is_independent_of_spec_count` with 0 vs 25 specs<br>final verification: `cargo test --manifest-path cli/Cargo.toml && cargo fmt --manifest-path cli/Cargo.toml --all -- --check && cargo clippy --manifest-path cli/Cargo.toml --all-targets -- -D warnings && cargo run --manifest-path cli/Cargo.toml -- freeze --check` | O(N) files only |
| AC-10 | cli | automated | QA-01 | active resolution and suggestion policy | PASS | exact expected-branch implementation; suggestion precedence unit test; full inspect CLI tests<br>final verification: `cargo test --manifest-path cli/Cargo.toml && cargo fmt --manifest-path cli/Cargo.toml --all -- --check && cargo clippy --manifest-path cli/Cargo.toml --all-targets -- -D warnings && cargo run --manifest-path cli/Cargo.toml -- freeze --check` | update never auto-suggested |
