...
| --- | --- | --- | --- | --- | --- | --- | --- |
| AC-01 | cli | automated | QA-01, QA-05, QA-06; conformance assertions for inline-only `build` and router wording | `engine/commands/build.md`, `engine/router.md`, `cli/crates/mochiflow-cli/tests/conformance.rs` | PASS | `cargo test --manifest-path cli/Cargo.toml`; targeted `build_is_inline_and_review_transport_is_reviewer_only`; final inline reviewer fallback full branch review: pass | Build and router now keep implementation inline while preserving task order, task commits, final verification, and AC settlement. |
| AC-02 | cli | automated | QA-01, QA-05; conformance assertions for review-only transport, cadence, and freshness | `engine/reference/risk.md`, `engine/commands/review.md`, `engine/agents/independent-reviewer.md`, `cli/crates/mochiflow-cli/tests/conformance.rs` | PASS | `cargo test --manifest-path cli/Cargo.toml`; targeted `build_is_inline_and_review_transport_is_reviewer_only`; final inline reviewer fallback full branch review: pass | Delegation remains only for independent review; cadence, plan-quality mode, and freshness are preserved. |
| AC-03 | cli | automated | QA-01, QA-05, QA-06; conformance assertions for inline `open` / `update` rework with no worker unit fields | `engine/commands/open.md`, `engine/commands/update.md`, `engine/reference/git.md`, `cli/crates/mochiflow-cli/tests/conformance.rs` | PASS | `cargo test --manifest-path cli/Cargo.toml`; targeted `inline_rework_lifecycle_and_adapter_lifecycle_are_specified`, `pr_feedback_routes_to_update_without_restore`; final inline reviewer fallback full branch review: pass | QA and PR-feedback code changes are bounded inline fixes without worker unit metadata or task trailers. |
| AC-04 | cli | automated | QA-03, QA-04, QA-05; adapter unit tests and CLI conformance for deprecated `spec-worker.json` removal/preservation behavior | deleted `engine/agents/worker.md`, deleted `engine/adapters/kiro/agents/spec-worker.json.tpl`, `engine/adapters/kiro/manifest.toml`, deleted `.kiro/agents/spec-worker.json`, `cli/crates/mochiflow-core/src/adapter.rs`, `cli/crates/mochiflow-cli/tests/conformance.rs` | PASS | `cargo test --manifest-path cli/Cargo.toml`; targeted `cargo test --manifest-path cli/Cargo.toml -p mochiflow-core adapter`; targeted `behavioral_kiro_retires_spec_worker_agent_and_self_heals`; `cargo run --manifest-path cli/Cargo.toml -- adapter generate` removed `.kiro/agents/spec-worker.json`; final inline reviewer fallback full branch review: pass | Markered worker output is self-healed by adapter generation; markerless files are preserved by tests. |
| AC-05 | cli | automated | QA-05, QA-06; `cargo test --manifest-path cli/Cargo.toml`, `cargo fmt --manifest-path cli/Cargo.toml --all -- --check`, `cargo clippy --manifest-path cli/Cargo.toml --all-targets -- -D warnings`, `cargo run --manifest-path cli/Cargo.toml -- freeze --check`, `mochiflow adapter generate --check` | `engine/MANIFEST.json`, `.mochiflow/engine/`, generated adapter outputs, `cli/crates/mochiflow-cli/tests/cli.rs`, `cli/crates/mochiflow-cli/tests/conformance.rs` | PASS | `mochiflow freeze`; `mochiflow upgrade --source engine`; `cargo run --manifest-path cli/Cargo.toml -- adapter generate --check`; `cargo run --manifest-path cli/Cargo.toml -- lint --spec retire-build-worker-orchestrator`; full default verification profile passed; final inline reviewer fallback full branch review: pass | Dogfood sync used the branch CLI for adapter self-heal because installed `mochiflow 1.1.3` did not include this branch's deprecated-output logic. |
| AC-06 | cli | automated | QA-02, QA-06; conformance assertions for session-recoverability wording and absence of worker-recoverability requirements | `engine/reference/authoring.md`, `engine/commands/plan.md`, `engine/agents/independent-reviewer.md`, `cli/crates/mochiflow-cli/tests/conformance.rs` | PASS | `cargo test --manifest-path cli/Cargo.toml`; targeted `session_recoverability_is_authoring_rule_not_lint`; final inline reviewer fallback full branch review: pass | Durable artifacts, committed code, and git trailers are the recoverability source set. |
