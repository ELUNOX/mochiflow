...
| AC-11 | cli | automated | QA-06 | `contracts/`, `engine/VERSION`, implementation diff | PASS | `git diff --name-only main...HEAD -- contracts contracts/contracts.lock engine/VERSION` returned empty; current unstaged diff for the same paths returned empty; version/contract conformance tests passed | No schema, lock, or version change |
| AC-12 | cli | automated | QA-03, QA-04, QA-06 | `cli/crates/mochiflow-cli/tests/conformance.rs` | PASS | `cargo test --manifest-path cli/Cargo.toml --test conformance` (170 passed) pins routing/deprecation, micro lint, Matrix template, removed no-PR path, and residual patch lane checks | New contract is pinned |
| AC-13 | cli | automated | QA-06 | `engine/MANIFEST.json`, `.mochiflow/engine/`, generated adapters, `cli/` | PASS | `mochiflow freeze`; `mochiflow upgrade --source engine`; `mochiflow adapter generate`; `mochiflow adapter generate --check`; `cargo test --manifest-path cli/Cargo.toml`; `cargo fmt --manifest-path cli/Cargo.toml --all -- --check`; `cargo clippy --manifest-path cli/Cargo.toml --all-targets -- -D warnings`; `cargo run --manifest-path cli/Cargo.toml -- freeze --check`; `mochiflow doctor` (0 fail); `mochiflow lint --spec retire-patch-for-micro-spec-depth` (0 fail) | Dogfood sync, doctor, and default verification |
