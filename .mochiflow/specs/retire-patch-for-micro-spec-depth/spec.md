...
| --- | --- | --- | --- | --- | --- |
| QA-01 | P1, P7 | cli | AI-observed | Inspect the no-active-spec small-fix route and the explicit `mochiflow-patch` token route. | Small-fix wording proposes planning; `mochiflow-patch` reports deprecation and routes toward plan. |
| QA-02 | P2 | cli | AI-observed | Inspect direct-micro plan intake, branch/draft commit ownership, and approval-card guidance. | The micro spec has `spec.yaml` + `spec.md`, no `pitch.md`/`design.md`/`tasks.md`; approval still follows the approve-to-build gate. |
...
| QA-05 | P5 | cli | AI-observed | Inspect the implementation diff for active/archived spec migration or rewrite. | Existing specs are not rewritten or migrated; historical artifacts remain readable. |
...
| QA-07 | P7 | cli | AI-observed | Compare edited engine docs, adapter outputs, README, lint behavior, and conformance tests against AC-01 through AC-13. | The documented workflow matches the spec; there are no residual active patch or no-PR instructions. |
...
| --- | --- | --- | --- | --- | --- | --- | --- |
| AC-01 | cli | automated + AI-observed | QA-01, QA-03, QA-07 | `engine/router.md`, adapter templates, generated adapter outputs, conformance tests | PASS | `cargo test --manifest-path cli/Cargo.toml --test conformance` (170 passed); `router_preserves_named_routing_branches`; `active_patch_lane_and_pr_bypass_residue_are_absent`; dogfood-regenerated `AGENTS.md` / `.kiro/steering/mochiflow.md` inspected | Small fixes route toward plan, not patch |
| AC-02 | cli | automated + AI-observed | QA-01, QA-03 | `engine/router.md`, conformance tests | PASS | `cargo test --manifest-path cli/Cargo.toml --test conformance` (170 passed); router contains only the retired `mochiflow-patch` branch and no `commands/patch.md` route | Deprecated token notice only |
| AC-03 | cli | automated + AI-observed | QA-02, QA-04 | `engine/commands/plan.md`, `engine/reference/workflow.md`, `engine/templates/spec/spec.micro.md`, lint tests | PASS | `direct_micro_plan_is_pitchless_and_branch_durable`; `micro_template_has_ac_verification_matrix`; `lint_passes_pitchless_micro_draft`; `lint_rejects_pitchless_draft_that_fails_micro_metadata` | Direct micro planning without pitch |
| AC-04 | cli | automated + AI-observed | QA-02, QA-06 | `engine/commands/plan.md`, `engine/reference/git.md`, conformance tests | PASS | `direct_micro_plan_is_pitchless_and_branch_durable` pins metadata derivation, confirmation, duplicate slug rejection, and `spec.yaml` creation | Direct micro intake creates confirmed metadata/spec.yaml |
| AC-05 | cli | automated + AI-observed | QA-02, QA-06 | `engine/commands/plan.md`, `engine/reference/git.md`, conformance tests | PASS | `direct_micro_plan_is_pitchless_and_branch_durable` pins branch creation/switch and verifies draft commit happens after draft lint and before approval; delegated reviewer confirmed the earlier ordering issue is resolved | Direct micro plan creates/switches branch and commits draft artifacts |
| AC-06 | cli | AI-observed + automated | QA-02, QA-07 | `engine/commands/plan.md`, `engine/reference/risk.md`, `engine/reference/git.md` | PASS | Static inspection of `plan.md`, `risk.md ## Micro escalation`, and `git.md`; conformance pins micro eligibility/escalation wording | Escalation criteria documented |
| AC-07 | cli | automated | QA-04 | `cli/crates/mochiflow-core/src/lint.rs`, conformance tests | PASS | `lint_passes_pitch_only_draft_spec`; `lint_passes_pitchless_micro_draft`; `lint_rejects_pitchless_draft_that_fails_micro_metadata`; `lint_rejects_pitchless_draft_that_fails_micro_file_shape`; full `cargo test --manifest-path cli/Cargo.toml` passed<br>final verification: `cargo test --manifest-path cli/Cargo.toml && cargo fmt --manifest-path cli/Cargo.toml --all -- --check && cargo clippy --manifest-path cli/Cargo.toml --all-targets -- -D warnings && cargo run --manifest-path cli/Cargo.toml -- freeze --check` | Three draft shapes plus documented ambiguity trade-off |
| AC-08 | cli | automated + AI-observed | QA-04, QA-07 | `engine/templates/spec/spec.micro.md`, `engine/commands/plan.md`, conformance tests | PASS | `micro_template_has_ac_verification_matrix`; `spec_templates_require_done_eligible_matrix_results`; `direct_micro_plan_is_pitchless_and_branch_durable` | Micro keeps AC Matrix traceability |
| AC-09 | cli | automated + AI-observed | QA-03, QA-06, QA-07 | `engine/reference/workflow.md`, `engine/reference/git.md`, `engine/commands/build.md`, `engine/commands/open.md`, `README.md`, `README.ja.md`, `CHANGELOG.md`, conformance tests | PASS | `pr_bypass_fast_path_is_removed`; `active_patch_lane_and_pr_bypass_residue_are_absent`; final dogfood sync and adapter check passed | No no-PR fast path remains |
| AC-10 | cli | automated + AI-observed | QA-03, QA-06 | `cli/crates/mochiflow-core/src/doctor.rs`, `cli/crates/mochiflow-core/src/adapter.rs`, `engine/MANIFEST.json`, conformance tests | PASS | `doctor_config_allows_current_cli_and_workflow_command_references`; `active_patch_lane_and_pr_bypass_residue_are_absent`; `worker_role_and_template_are_retired`; `mochiflow doctor` reported 0 fail | Patch removed from active command catalog |
| AC-11 | cli | automated | QA-06 | `contracts/`, `engine/VERSION`, implementation diff | PASS | `git diff --name-only main...HEAD -- contracts contracts/contracts.lock engine/VERSION` returned empty; current unstaged diff for the same paths returned empty; version/contract conformance tests passed<br>final verification: `cargo test --manifest-path cli/Cargo.toml && cargo fmt --manifest-path cli/Cargo.toml --all -- --check && cargo clippy --manifest-path cli/Cargo.toml --all-targets -- -D warnings && cargo run --manifest-path cli/Cargo.toml -- freeze --check` | No schema, lock, or version change |
| AC-12 | cli | automated | QA-03, QA-04, QA-06 | `cli/crates/mochiflow-cli/tests/conformance.rs` | PASS | `cargo test --manifest-path cli/Cargo.toml --test conformance` (170 passed) pins routing/deprecation, micro lint, Matrix template, removed no-PR path, and residual patch lane checks<br>final verification: `cargo test --manifest-path cli/Cargo.toml && cargo fmt --manifest-path cli/Cargo.toml --all -- --check && cargo clippy --manifest-path cli/Cargo.toml --all-targets -- -D warnings && cargo run --manifest-path cli/Cargo.toml -- freeze --check` | New contract is pinned |
| AC-13 | cli | automated | QA-06 | `engine/MANIFEST.json`, `.mochiflow/engine/`, generated adapters, `cli/` | PASS | `mochiflow freeze`; `mochiflow upgrade --source engine`; `mochiflow adapter generate`; `mochiflow adapter generate --check`; `cargo test --manifest-path cli/Cargo.toml`; `cargo fmt --manifest-path cli/Cargo.toml --all -- --check`; `cargo clippy --manifest-path cli/Cargo.toml --all-targets -- -D warnings`; `cargo run --manifest-path cli/Cargo.toml -- freeze --check`; `mochiflow doctor` (0 fail); `mochiflow lint --spec retire-patch-for-micro-spec-depth` (0 fail)<br>final verification: `cargo test --manifest-path cli/Cargo.toml && cargo fmt --manifest-path cli/Cargo.toml --all -- --check && cargo clippy --manifest-path cli/Cargo.toml --all-targets -- -D warnings && cargo run --manifest-path cli/Cargo.toml -- freeze --check` | Dogfood sync, doctor, and default verification |
