...
prerelease の間はバージョン付き URL を使います。例:
...
プロジェクトの現状マップはコードから再生成（**refresh**）されます。
...
`.mochiflow/context/product.md` と `.mochiflow/context/structure.md` のコード由来の補完、
adapter 再生成、`mochiflow doctor` まで実行させます。adapter ファイルの手動統合が
...
残り（`.mochiflow/config.toml`・`.mochiflow/specs/`・`.mochiflow/memory/`・
`.mochiflow/INDEX.md`）は追跡対象です。
...
- `engine/VERSION` — source engine の semver。
...
- `mochiflow --version` — 製品 / CLI のバージョン。CLI は `mochiflow upgrade` が使う同梱 engine も持ちます。
