...
<p align="center">
  AIコーディングエージェントに、相談・設計・実装・PR提出の流れを渡すSpecワークフロー。
</p>
...
| PRを作成する | AIツールに「PRを作成して」、または `mochiflow-open` |

`mochiflow-discuss`、`mochiflow-plan`、`mochiflow-build`、`mochiflow-open`、`mochiflow-update`、`mochiflow-close` はターミナルで実行するコマンドではありません。
...
PRレビューで修正を求められたら、更新に進めます。

```text
mochiflow-update
```

AIエージェントはフィードバックを実装ループで反映し、再検証してpushし、PRを更新します。
specはそのままで、巻き戻しもアーカイブもしません。

### 6. マージ後に後片付けする

PRがマージされたら、後片付けに進めます。

```text
mochiflow-close
```

MochiFlowはローカルの後片付けだけを行います（ベースブランチへ戻り、fast-forward、
ローカルブランチ削除、ボード再生成）。ベースブランチには何も書き込みません。
...
| AIツール | 生成される入口 | 役割 |
| --- | --- | --- |
| Kiro | `.kiro/` | 常時読み込みsteering + 読み取り専用レビューア |
...
mochiflow status                       # 配信ボードを表示（Backlog / Active / Ready / In Review / Done、読み取り専用）
